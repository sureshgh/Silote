...
class MyFrame1 ( wx.Frame ):
	...
	def __init__( self, parent, myTitle ):
		...
		m_comboBox3Choices = ["300", "600", "1200", "2400", "4800", "9600", "19200", "38400", "57600", "115200"]
		...
		self.m_richText1.SetKeyWords(0,'setup loop pinMode digitalWrite digitalRead analogWrite analogRead OUTPUT INPUT int void char for while do long short unsigned signed if else switch case break default continue Serial available print DEC read BIN write println delay HIGH LOW')
	...
	def onSave(self, evt):
		...
		if (currentWorkingFile == "untitled"):
			...
			frame.SetTitle (u"Simple Arduino IDE - " + saveFileName)
	...
	def onOpen(self, evt):
		...
		frame.SetTitle (u"Simple Arduino IDE - " + openFileName)
		self.m_button9.SetLabel("Save")

	def onNew(self, evt):
		newFileName = 'python "' + os.getcwd() + '/ideSimple.py" &'
	...
	def onCompile(self, evt):
		...
		compileString2 = 'avr-gcc -Os -Wl,--gc-sections -mmcu=%s -o %s.elf %s.o "%s/karkhuino/cores/%s" -lm' % (mmcu, fileName, fileName, os.getcwd(), aName)
	...
	def onUpload ( self, evt ):
		...
		compileString1 = '"%s/karkhuino/tools/avrdude" -C"%s/karkhuino/tools/avrdude.conf" -p%s -carduino -P%s -b%s -D -Uflash:w:%s.hex:i' % (os.getcwd(), os.getcwd(), mmcu, serialPort , bspeed, fileName)
		os.system (compileString1)
		...
		while (1):
			...
			if (start == True):
				...
				if (words[0] == "mmcu"):
					...
					core = '"' + os.getcwd() + '/karkhuino/arduino/cores/' + words[1] + '"'
				elif (words[0] == "variant"):
					variant = '"' + os.getcwd() + "/karkhuino/arduino/variants/" + words[1] + '"'
				elif (words[0] == "bspeed"):
					bspeed = words[1]
				elif (words[0] == "aname"):
					aName = words[1]
...
if __name__ == '__main__':
	app = wx.App()
	frame = MyFrame1(None, u"Simple Arduino IDE - " + currentWorkingFile )
	frame.Center()
	frame.Show()
	app.MainLoop()
